...
void FLeapPlugin::StartupModule()
{
...
void FLeapPlugin::ShutdownModule()
{
...
LeapControllerData* FLeapPlugin::ControllerData()
{
	if (bActive) 
	{
...
void FLeapPlugin::AddEventDelegate(UObject* EventDelegate)
{
	LeapInputDevice->AddEventDelegate(EventDelegate);
}


void FLeapPlugin::RemoveEventDelegate(UObject* EventDelegate)
{
	LeapInputDevice->RemoveEventDelegate(EventDelegate);
}

TSharedPtr< class IInputDevice > FLeapPlugin::CreateInputDevice(const TSharedRef< FGenericApplicationMessageHandler >& InMessageHandler)
{
	...
	FLeapPlugin::LeapInputDevice = MakeShareable(new FLeapInputDevice(InMessageHandler));
...
IMPLEMENT_MODULE(FLeapPlugin, Leap)
